...
- **Admin → Judging → Pitch order:** move projects up or down and save. Judges see their assigned subset in that order. New projects go at the end.
- **Admin → Judging → Judge assignments:** assign each judge a subset and mark conflicts. Every submitted, active project needs at least one non-conflicted judge.
