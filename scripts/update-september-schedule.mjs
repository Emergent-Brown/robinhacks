#!/usr/bin/env node
/** One-time, pre-event schedule publication. Reruns preserve organizer edits. */
...
async function main() {
  ...
  assert(
    ...
    'Usage: node --env-file=.env.operations scripts/update-september-schedule.mjs --project <configured-project> [--apply]',
  );
  ...
  const app = initializeApp({ projectId: project, credential }, 'schedule-publication');
  ...
  try {
    ...
    const marker = root.collection('migrations').doc('september-2026-schedule-short-rounds');
    const report = await db.runTransaction(async (tx) => {
      const [snapshot, membership, previous] = await Promise.all([
        tx.get(root),
      ...
      if (apply) {
        ...
        tx.create(root.collection('adminAudit').doc('september-2026-schedule-short-rounds'), {
          id: 'september-2026-schedule-short-rounds',
          actorUid: owner.uid,
          action: 'publishSchedule',
          detail:
            'Corrected investment windows to 30 minutes and removed internal details from the public schedule. Funding rules and live state preserved.',
          createdAt: now,
        });
      }
      return {
...
main().catch(() => {
  console.error(
    'Schedule publication stopped. Check the existing organizer login, configured project and registration phase. No credential details were printed.',
  );
  process.exitCode = 1;
});
